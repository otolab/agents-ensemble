...
- **CLI デフォルト**: TTY、CLI 初回メッセージ、または `ENSEMBLE_OPERATOR_MESSAGE` あり → 無制限。非 TTY / CI → 5（暴走防止）
...
CLI: TTY は Ink TUI（非ブロッキング入力 + `pane` / `stream` レイアウト）、非 TTY は CLI 初回メッセージ / `ENSEMBLE_OPERATOR_MESSAGE` / `bindAsyncOperatorInput`。ログ・表示の正本は [session-logging.md](session-logging.md)。対話モデルは [ADR 0008](adr/0008-human-dialogue-open-questions.md)。
