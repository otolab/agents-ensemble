...
Open questions は内容駆動で高さを決めます。選択中 question の `question` と `context` は折り返し後の要求行数として扱い、非選択 question は 1 行の compact 表示として同じ一覧に残します。pane / stream とも、まずこの要求行数に合わせて Open questions 枠を拡大し、通常の 80×24 程度では質問本文と compact 行を読める範囲を確保します。端末の高さまたは本文上限を超える極端に長い内容は、既存の短端末向け compact / clip 方針に従って固定ペインや本文を縮めます。Open questions ペイン内のスクロールは行いません。
