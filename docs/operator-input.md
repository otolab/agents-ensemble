...
| GitHub monitor `stop`（accepted risk） | 監視有効時、`gh` poll 実行中（`pollInFlight`）に `/exit` すると `stop()` が poll 完了までブロックしうる。**タイムアウト未実装** | 「終了しています…」のまま長時間残る可能性。`harness.teardown` は出ない |
...
| 症状 | UI は即「終了しています…」になるが、teardown が `githubMonitor.stop()` で `pollInFlight` 待ちになりプロセスが長時間残る |
| 回避策 | `--no-github-monitor` で監視を無効化（[README.md](../README.md)・[harness-events.md](harness-events.md)） |
| 未実装 | poll / `stop()` のタイムアウト（別 Issue で検討可） |
