...
| resize settle | `100ms` (`TUI_RESIZE_SETTLE_MS`) |
...
| テスト | 確認する契約 | 合格条件 |
|--------|--------------|----------|
| [`tui-terminal-size.test.ts`](../packages/cli/src/tui/tui-terminal-size.test.ts) | settle window が resize burst を 1 つの snapshot にまとめる。`createTuiResizeController` 経由で Ink の resize 通知と TUI の frame が同じ settled size を使う | `100ms` 前の通知が遅延し、settle 後に 1 回だけ通知される。実 Ink frame の resize 後出力に pane の主要な枠・入力行があり、`ESC[2J` / `ESC[3J` が出力されない |
